...
SELECT 
    ...
    -- Live Expected Cash (Sum of all CASH debits - sum of all CASH credits)
    COALESCE((
        SELECT SUM(al.debit - al.credit) FROM public.accounting_ledger al 
        WHERE al.shift_id = s.id AND al.payment_method = 'CASH'
