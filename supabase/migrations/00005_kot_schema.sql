-- 1. Add fulfillment_status to orders
ALTER TABLE public.orders 
ADD COLUMN fulfillment_status TEXT NOT NULL CHECK (fulfillment_status IN ('NEW', 'PREPARING', 'READY', 'COMPLETED')) DEFAULT 'NEW';

-- 2. Add fulfillment_status to order_items
ALTER TABLE public.order_items 
ADD COLUMN fulfillment_status TEXT NOT NULL CHECK (fulfillment_status IN ('PENDING', 'DONE')) DEFAULT 'PENDING';

-- 3. Replace the checkout_order RPC to include fulfillment_status
...
BEGIN
  ...
  -- 2. Insert Order (with fulfillment_status = 'NEW')
  v_cashier_id := auth.uid();
  v_subtotal := (payload->>'subtotal')::NUMERIC;
  v_tax := (payload->>'tax')::NUMERIC;
  v_discount := (payload->>'discount')::NUMERIC;

  INSERT INTO public.orders (cashier_id, status, subtotal, tax, discount, total, fulfillment_status)
  VALUES (v_cashier_id, 'PAID', v_subtotal, v_tax, v_discount, v_total, 'NEW')
  RETURNING id, order_number INTO v_order_id, v_order_number;

  -- 3. Insert Items and Modifiers (with fulfillment_status = 'PENDING')
