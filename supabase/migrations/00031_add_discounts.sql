...
ALTER TABLE public.orders ADD COLUMN discount_type TEXT CHECK (discount_type IN ('percentage', 'amount')) DEFAULT 'percentage';
ALTER TABLE public.orders ADD COLUMN discount_value NUMERIC(10,2) DEFAULT 0.00;

-- Add enable_discount to store_settings table
ALTER TABLE public.store_settings ADD COLUMN enable_discount BOOLEAN NOT NULL DEFAULT TRUE;

-- Update checkout_order RPC to store discount type and value
...
BEGIN
  ...
  v_discount_type := COALESCE(payload->>'discount_type', 'percentage');
  v_discount_value := COALESCE((payload->>'discount_value')::NUMERIC, 0.00);

  INSERT INTO public.orders (cashier_id, shift_id, status, subtotal, tax, discount, total, fulfillment_status, discount_type, discount_value)
  VALUES (v_cashier_id, v_shift_id, 'PAID', v_subtotal, v_tax, v_discount, v_total, 'NEW', v_discount_type, v_discount_value)
