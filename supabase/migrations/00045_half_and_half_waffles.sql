...
-- 1. Add allow_half_and_half to products
ALTER TABLE public.products ADD COLUMN IF NOT EXISTS allow_half_and_half BOOLEAN DEFAULT false;

-- 2. Add metadata JSONB to order_items
ALTER TABLE public.order_items ADD COLUMN IF NOT EXISTS metadata JSONB;

-- 3. Add half_and_half_surcharge to store_settings
ALTER TABLE public.store_settings ADD COLUMN IF NOT EXISTS half_and_half_surcharge NUMERIC(10,2) DEFAULT 0.00;

-- 4. Rewrite create_unpaid_order to support half and half inventory deduction
...
BEGIN
  ...
  LOOP
    INSERT INTO public.order_items (
      order_id, product_id, product_name_snapshot, unit_price_snapshot, quantity, subtotal, fulfillment_status, notes, metadata
    ) VALUES (
      v_order_id,
    ...
    -- INVENTORY DEDUCTION
    IF (v_item->'metadata'->>'type') = 'half_and_half' THEN
      FOR v_half IN SELECT * FROM jsonb_array_elements(v_item->'metadata'->'halves')
      LOOP
        INSERT INTO public.inventory_transactions (ingredient_id, transaction_type, quantity_changed, reference_id, notes)
        SELECT r.ingredient_id, 'SALE', -0.5 * r.quantity_required * (v_item->>'quantity')::INT, v_order_id, 'Sold Half: ' || (v_half->>'product_name')
        FROM public.recipes r
        WHERE r.product_id = (v_half->>'product_id')::UUID;
        
        UPDATE public.ingredients i
        SET current_stock = i.current_stock - (0.5 * r.quantity_required * (v_item->>'quantity')::INT),
            updated_at = NOW()
        FROM public.recipes r
        WHERE r.product_id = (v_half->>'product_id')::UUID AND i.id = r.ingredient_id;
      END LOOP;
    ELSE
      INSERT INTO public.inventory_transactions (ingredient_id, transaction_type, quantity_changed, reference_id, notes)
...
-- 5. Rewrite checkout_order to support half and half inventory deduction
